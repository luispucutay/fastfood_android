...
package com.hcpt.fastfood.widget;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private List<View> tabsList = new ArrayList<View>();
	...
	public ArrayList<TabItem> getArrTabs() {
		return arrTabs;
	}

	public void setArrTabs(ArrayList<TabItem> arrTabs) {
	...
	public void setListIconTabs(ArrayList<TabItem> arrTab) {
		arrTabs = arrTab;
	}
	...
	private void addTextTab(final int position) {
		...
		if (!arrTabs.isEmpty()) {
			...
			Drawable iconDrawable = getResources().getDrawable(
					arrTabs.get(position).getIcon());
			tab.setCompoundDrawablesWithIntrinsicBounds(null, iconDrawable,
					null, null);
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(0, 0, tabsContainer.getWidth(), height, rectPaint);
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (i != currentPosition) {
				...
				icon = getResources().getDrawable(arrTabs.get(i).getIcon());
			} else {
				text.setTextColor(selectedTextColor);
				icon = getResources().getDrawable(arrTabs.get(i).getSelected());
